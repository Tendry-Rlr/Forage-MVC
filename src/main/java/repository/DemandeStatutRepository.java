...
public interface DemandeStatutRepository extends JpaRepository<DemandeStatut, Integer> {
    ...
    @Query("select ds from DemandeStatut ds where ds.demande.id_demande = :demandeid order by ds.date asc")
    List<DemandeStatut> findByIdDemandeDateASC(@Param("demandeid") Integer demandeid);
    ...
    @Query("select sum(ds.duree_travaille) from DemandeStatut ds where ds.demande.id_demande = :idd")
    double sommeDT(@Param("idd") Integer idd);
